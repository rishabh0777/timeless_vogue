...
const Navbar = () => {
  const { cartLength, setCart } = useContext(DataContext);
  ...
  const handleLogout = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${url}/api/v1/user/logout`, {}, {
  ...
  return (
    <>
      <nav className="w-full h-[10vh] bg-white fixed top-0 z-[1000] shadow-md px-4 sm:px-6 md:px-10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="md:hidden">
            <i className="ri-menu-3-line text-2xl cursor-pointer" onClick={() => setIsMenuOpen(true)}></i>
          </div>
        </div>
      </nav>
      ...
      <div
        ref={optionRef}
        className="w-[150px] bg-black text-white flex-col gap-2 justify-center items-start rounded-lg z-[1000] fixed right-6 top-[9.5vh] py-2 px-4 hidden shadow-md"
      >
        <h3
          onClick={() => {
            navigate('/profile');
        ...
        <p
          ...
          className="flex items-center gap-2 cursor-pointer"
        >
          <i className="ri-shopping-bag-4-fill"></i> Orders
        </p>
        <p className="flex items-center gap-2 cursor-pointer">
          <i className="ri-moon-clear-line"></i> Dark
        </p>
      ...
      {/* Mobile Slide Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-[70vw] bg-white z-[1100] p-6 shadow-lg transition-transform duration-300 transform ${
