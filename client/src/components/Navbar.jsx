...
const Navbar = () => {
  ...
  useEffect(() => {
    // Simulate loading user/cart info
    ...
    <>
      <nav className="w-full h-[10vh] px-12 flex items-center justify-between fixed top-0 bg-white z-[1000] shadow-lg">
        ...
        <div className='flex gap-[3vw]'>
          <div className='flex relative' onClick={() => navigate('/cart')}>
            <i className="ri-shopping-bag-line cursor-pointer"></i>
            {
              cartLength > 0 && (
                <p className='h-[1.5vw] w-[1.5vw] flex justify-center items-center rounded-full text-[0.9vw] absolute left-2 top-[-1.2vh] bg-zinc-800 text-white'>
                  {cartLength}
                </p>
              )
            }
